...
<table class="table">
    ...
    <tr><td>{{Lang::get('messages.createdAt')}}</td>
        <td>{{ $cita->created_at }}</td></tr>
</table>
...
@else
    ...
    @foreach ($cita->contactos as $contacto)
            <li><a href="{{route('contactos.show', $contacto->slug) }}">
                    {{ $contacto->nom }}</a></li>
    @endforeach
    </dl>
@endif
...
{!! Form::submit(Lang::get('messages.addContact'), 
array('class' => 'btn btn-info')) !!}
{!! Form::close() !!}
{!! link_to_route('citas.edit',Lang::get('messages.editMeeting'), 
array($cita->slug)) !!}<br/>
